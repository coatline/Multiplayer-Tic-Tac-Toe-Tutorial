...
public class LobbyManager : Singleton<LobbyManager>
{
    ...
    IntervalTimer lobbyPollTimer;
    IntervalTimer refreshLobbyListTimer;
    ...
    protected override void Awake()
    {
        ...
        heartbeatTimer = new IntervalTimer(15f, true);
        lobbyPollTimer = new IntervalTimer(1.1f, true);
        refreshLobbyListTimer = new IntervalTimer(5f, true);
    }


    private void Update()
    {
        //HandleRefreshLobbyList(); // Disabled Auto Refresh for testing with multiple builds or we would hit the rate limit
    ...
    private void HandleRefreshLobbyList()
    {
        ...
        {
            if (refreshLobbyListTimer.DecrementIfRunning(Time.deltaTime))
            {
                refreshLobbyListTimer.Start();
                RefreshLobbyList();
            }
        }
    }
    ...
    private async void HandleLobbyHeartbeat()
    {
        if (IsLobbyHost())
        {
            if (heartbeatTimer.DecrementIfRunning(Time.deltaTime))
            {
    ...
    private async void HandleLobbyPolling()
    {
        if (joinedLobby != null)
        {
            if (lobbyPollTimer.DecrementIfRunning(Time.deltaTime))
            {
                lobbyPollTimer.Start();
    ...
    {
        Debug.Log("Join Game " + relayJoinCode);
